...
# Majority of this Rakefile was pulled from Octopress. imathis has done some excellent work!
...
blog_index_dir  = 'src'    # directory for your blog's index page (if you put your index in source/blog/index.html, set this to 'source/blog')
...
task :run do
  ...
  foremanPid = Process.spawn("foreman start -f=Procfile_local")
   ...
   trap("INT") {
    [jekyllPid, foremanPid].each { |pid| Process.kill(9, pid) rescue Errno::ESRCH }
    exit 0
  }

  [jekyllPid, foremanPid].each { |pid| Process.wait(pid) }
end
...
task :setup_github_pages, :repo do |t, args|
  ...
  cd "#{deploy_dir}" do
    system "git init"
    system 'echo "My Jekyll Page is coming soon &hellip;" > index.html'
    system "git add ."
    system "git commit -m \"StarGazer init\""
...
def blog_url(user, project)
  url = if File.exists?('src/CNAME')
    "http://#{IO.read('src/CNAME').strip}"
  else
